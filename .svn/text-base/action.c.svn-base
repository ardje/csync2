...
found_matching_pattern:
			for (c=a->command; c; c=c->next)
				csdbAddActionFilename_Command_Logfile_(
					url_encode(filename), url_encode(c->command), url_encode(a->logfile));
		}
	}
}
...
void csync_run_single_command(const char *command, const char *logfile)
{
	...
	tl=csdbGetActionCommand_Logfile_(command,logfile);
	...
	for (t = tl; t != 0; t = t->next)
		csdbDeleteActionFilename_Command_Logfile_(t->value,command,logfile);

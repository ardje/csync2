...
{
	struct textlist *dirtylist;
	struct textlist *tl = 0, *t, *next_t;
	struct textlist *tl_mod = 0, **last_tn=&tl;
	char *current_name = 0;
	struct stat st;

	dirtylist=csdbGetDirtyFilesPeername_Maxamount_(url_encode(peername),csync_limit_work);
	for(t=dirtylist;t;t=t->next) {
		int i, use_this = patnum == 0;
		const char *filename = url_decode(t->value);
		for (i=0; i<patnum && !use_this; i++)
			if ( compare_files(filename, patlist[i], recursive) ) use_this = 1;
		if (use_this)
			textlist_add2(&tl, filename, url_decode(t->value2), t->intvalue);
	}
	textlist_free(dirtylist);
...
{
	struct textlist *cf,*filelist=0,*diff_list = 0, *diff_ent;
...
found_host:
	...
	filelist=csdbGetFilesFilename_(filename?url_encode(filename):filename);
	for(cf=filelist;cf;cf=cf->next) {
		char *l_file = strdup(url_decode(cf->value2)), *l_checktxt = strdup(url_decode(cf->value));
...
{
	struct textlist *tl = 0, *t;
	struct textlist *dirtylist, *filelist;
	dirtylist=csdbGetDirty();
	...
	for(t=dirtylist;t;t=t->next) {
		...
		while ((g=csync_find_next(g, filename)) != 0) {
			if (!strcmp(g->myname, t->value2))
				for (h = g->host; h; h = h->next) {
					if (!strcmp(h->hostname, t->value3))
						goto this_dirty_record_is_ok;
				}
		}

		textlist_add2(&tl, t->value, t->value3, 0);
...
this_dirty_record_is_ok:
	...
	for (t = tl; t != 0; t = t->next) {
		...
		csdbDeleteDirtyFilename_Peername_(t->value, t->value2);
	}
	textlist_free(tl);

	tl = 0;
	filelist=csdbGetFilesNamesonly();
	for(t=filelist;t;t=t->next) {
		if (!csync_find_next(0, url_decode(t->value)))
			textlist_add(&tl, t->value, 0);
	}
	textlist_free(filelist);
	for (t = tl; t != 0; t = t->next) {
		csync_debug(1, "Removing %s from file db.\n", t->value);
		csdbDeleteFileFilename_(t->value);
	}
	textlist_free(tl);
}

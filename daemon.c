...
int csync_check_dirty(const char *filename, const char *peername, int isflush)
{
	...
	if (isflush) return 0;
	count=csdbCheckDirtyFilename_(url_encode(filename));
	if(count>0) {
		rc = 1;
...
{
	struct stat st;
	csdbDeleteDirtyFilename_Peername_(url_encode(filename),peername);
	if ( lstat_strict(prefixsubst(filename), &st) != 0 || csync_check_pure(filename) ) {
		csdbDeleteFileFilename_(url_encode(filename));
	} else {
		const char *checktxt = csync_genchecktxt(&st, filename, 0);
		csdbAddFileFilename_Check_(url_encode(filename), url_encode(checktxt));
	}
}

void csync_file_flush(const char *filename)
{
	csdbDeleteDirtyFilename_Peername_(url_encode(filename),NULL);
}
...
void csync_daemon_session()
{
	...
	while ( conn_gets(line, 4096) ) {
		...
		case A_GETSZ:
			...
			break;
		case A_FLUSH:
			csdbDeleteDirtyFilename_Peername_(url_encode(tag[2]),NULL);
			break;
		case A_DEL:
		...
		case A_LIST: {
			...
			tl=csdbGetFilesFilename_(fn);
			for(t=tl;t;t=t->next) {
				if ( csync_match_file_host(url_decode(t->value2), tag[1], peer, (const char **)&tag[3]) )
					conn_printf("%s\t%s\n", t->value, t->value2);

...
{
	csdbAddHintFilename_Flags_(url_encode(file),recursive);
}
...
void csync_mark(const char *file, const char *thispeer, const char *peerfilter)
{
	...
	for (pl_idx=0; pl[pl_idx].peername; pl_idx++)
		if (!peerfilter || !strcmp(peerfilter, pl[pl_idx].peername))
			csdbAddDirtyFilename_Flags_Myname_Peername_(
				url_encode(file),
				csync_new_force?DIRTY_FORCE:0,
...
void csync_check_del(const char *file, int recursive, int init_run)
{
	...
	if ( recursive ) {
			csync_debug_ping(2);
		ftl=csdbGetFilesRecursiveFilename_(encoded_file);
			csync_debug_ping(2);
	} else {
			csync_debug_ping(2);
		ftl=csdbGetFilesFilename_(encoded_file);
			csync_debug_ping(2);
	}
			csync_debug_ping(2);
	for(t=ftl;t;t=t->next) {
		const char *filename = url_decode(t->value2);
	...
	for (t = tl; t != 0; t = t->next) {
		...
		csdbDeleteFileFilename_(url_encode(t->value));
	}
			csync_debug_ping(2);
...
int csync_check_mod(const char *file, int recursive, int ignnoent, int init_run)
{
	...
	case 2: {
		...
		tl=csdbGetFilesFilename_(url_encode(file));
		if(tl) {
			for(t=tl;t;t=t->next) {
		...
		if ( this_is_dirty && !csync_compare_mode ) {
			csdbAddFileFilename_Check_(url_encode(file), url_encode(checktxt));
